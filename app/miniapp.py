...
def _source_rows(items: list[dict], stage_key: str) -> list[dict]:
    ...
    for source, source_items in sorted(grouped.items(), key=lambda pair: (-len(pair[1]), pair[0])):
        stages = Counter(item[stage_key] for item in source_items)
        result.append({
            "source": source,
...
def report_payload(data: dict) -> dict:
    ...
    return {
        ...
        "leads": _source_rows(leads, "result_stage"),
        "deals": _source_rows(deals, "stage"),
    }
